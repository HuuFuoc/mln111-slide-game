...
void main(){gl_Position=position;}`;
    ...
    const loop = (now: number) => {
      ...
      const pCount = gl.getUniformLocation(program, 'pointerCount');
      const pCoords = gl.getUniformLocation(program, 'pointers');

      gl.uniform2f(res, canvas.width, canvas.height);
      gl.uniform1f(time, now * 1e-3);
      gl.uniform2f(move, mouseMove[0], mouseMove[1]);
      gl.uniform2f(touch, mouseCoords[0], mouseCoords[1]);
      gl.uniform1i(pCount, pointerCount);
      gl.uniform2fv(pCoords, pointerCoords);
    ...
    const onPointerMove = (e: PointerEvent) => {
      if (!active) return;
      pointers.set(e.pointerId, map(e.clientX, e.clientY));
      mouseCoords = pointers.values().next().value ?? [0, 0];
      mouseMove = [mouseMove[0] + e.movementX, mouseMove[1] + e.movementY];
      pointerCoords = Array.from(pointers.values()).flat();
      pointerCount = pointers.size;
    };
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-white">
        ...
        <div className="text-center space-y-6 max-w-5xl mx-auto px-4">
          <div className="space-y-2">
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold bg-gradient-to-r from-[#fefae0] via-[#dda15e] to-[#bc6c25] bg-clip-text text-transparent animate-fade-in-up animation-delay-200">
              {headline.line1}
            </h1>
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold bg-gradient-to-r from-[#dda15e] via-[#bc6c25] to-[#bc4749] bg-clip-text text-transparent animate-fade-in-up animation-delay-400">
              {headline.line2}
            </h1>
          </div>
